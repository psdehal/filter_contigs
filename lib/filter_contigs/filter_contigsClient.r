...
filter_contigsClient <- function(url, token = NULL, flatten = TRUE, 
    ...
    ret_client_object_ref[['count_contigs']] <- function(workspace_name, contigset_id) {
        ret <- ret_client_object_ref[['json_rpc_call']]("filter_contigs.count_contigs", list(
                workspace_name, contigset_id
        ))
        return(ret[[1]])
    }

    ret_client_object_ref[['filter_contigs_by_length']] <- function(input_params) {
        ret <- ret_client_object_ref[['json_rpc_call']]("filter_contigs.filter_contigs_by_length", list(
                input_params
        ))
        return(ret[[1]])
    }
    ...
    class(ret_client_object_ref) <- 'filter_contigsClient'
