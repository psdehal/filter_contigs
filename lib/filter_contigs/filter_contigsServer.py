...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'filter_contigs'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from filter_contigs.filter_contigsImpl import filter_contigs
impl_filter_contigs = filter_contigs(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['filter_contigs.count_contigs_async'] = ['filter_contigs', 'count_contigs']
async_check_methods['filter_contigs.count_contigs_check'] = ['filter_contigs', 'count_contigs']
sync_methods['filter_contigs.count_contigs'] = True
async_run_methods['filter_contigs.filter_contigs_by_length_async'] = ['filter_contigs', 'filter_contigs_by_length']
async_check_methods['filter_contigs.filter_contigs_by_length_check'] = ['filter_contigs', 'filter_contigs_by_length']
sync_methods['filter_contigs.filter_contigs_by_length'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'filter_contigs'
        ...
        self.rpc_service.add(impl_filter_contigs.count_contigs,
                             name='filter_contigs.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['filter_contigs.count_contigs'] = 'required'
        self.rpc_service.add(impl_filter_contigs.filter_contigs_by_length,
                             name='filter_contigs.filter_contigs_by_length',
                             types=[dict])
        self.method_authentication['filter_contigs.filter_contigs_by_length'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "filter_contigs but no authentication header was passed"
                            raise err
