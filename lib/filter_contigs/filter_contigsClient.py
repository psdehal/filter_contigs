...
class filter_contigs(object):
    ...
    def count_contigs(self, workspace_name, contigset_id, json_rpc_context = None):
        ...
        resp = self._call('filter_contigs.count_contigs',
                          [workspace_name, contigset_id], json_rpc_context)
        return resp[0]
  
    def filter_contigs_by_length(self, input_params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method filter_contigs_by_length: argument json_rpc_context is not type dict as required.')
        resp = self._call('filter_contigs.filter_contigs_by_length',
